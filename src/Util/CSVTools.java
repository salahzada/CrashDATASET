...
public class CSVTools {
	...
	return filter_string(rows, regex, false, fields);
    }

    public static List<String[]> filter_string(List<String[]> rows, String regex, boolean noCase, String... fields) {
	...
	outer: for (int i = 1; i < rows.size(); i++) {
		...
		if (!includeHeaders[j])
		    continue;
		String s = (noCase) ? row[j].toLowerCase() : row[j];
    ...
    public static String year2Date(int y) {
	return "01/01/" + y;
    }
     ...
     */
    public static List<String[]> filter_dates(List<String[]> rows, int num, int mode) {

	List<String[]> retval = new ArrayList<>();
	retval.add(rows.get(0));
	outer: for (int i = 1; i < rows.size(); i++) {
	    String[] row = rows.get(i);
	    if (num > 1000) {
		row[0] = convertToBiggerDate(row[0]);
	    }

	    String date[] = row[0].split("/");
	    int d = Integer.parseInt(date[mode - 1]);
	...
	outer: for (int i = 1; i < rows.size(); i++) {
		 ...
		 * 2: "greater than", // <br>
		 * 3: "less than", // <br>
		 * 4: "greater and equal to", // <br>
		 * 5: "less and equal to", // <br>
		 * 6: "between", // <br>
		 * 7: "null"
		 */
		switch (op) {
		case "eq":
		    if (Double.parseDouble(row[j]) == Double.parseDouble(key)) {
			retval.add(row);
			continue outer;
		    }
		    break;
		...
		case "bt":
		    ...
		    double c = Double.parseDouble(row[j]);
		    if (c >= a && c <= b) {
			retval.add(row);
			continue outer;
		    }
